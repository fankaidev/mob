...
import { type CustomCommand, createLazyCustomCommand, isLazyCommand } from "./custom-commands";
...
import { ArithmeticError, ExecutionLimitError, ExitError, PosixFatalError } from "./interpreter/errors";
import { buildBashopts, buildShellopts } from "./interpreter/helpers/shellopts";
import { Interpreter, type InterpreterOptions, type InterpreterState } from "./interpreter/index";
import { type ExecutionLimits, resolveLimits } from "./limits";
import { createSecureFetch, type NetworkConfig, type SecureFetch } from "./network/index";
import { LexerError } from "./parser/lexer";
import { type ParseException, parse } from "./parser/parser";
import { DefenseInDepthBox, SecurityViolationError } from "./security/defense-in-depth-box";
import type { DefenseInDepthConfig } from "./security/types";
import type { BashExecResult, Command, CommandRegistry, FeatureCoverageWriter, TraceCallback } from "./types";
...
export class Bash {
	...
	constructor(options: BashOptions = {}) {
		...
		if (cwd !== "/" && typeof (fs as any).mkdirSync === "function") {
			try {
				(fs as any).mkdirSync(cwd, { recursive: true });
			} catch {
				// Ignore errors
			}
		}
