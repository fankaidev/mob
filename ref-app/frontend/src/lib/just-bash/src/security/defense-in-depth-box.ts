...
import { type BlockedGlobal, getBlockedGlobals } from "./blocked-globals";
import type {
...
// AsyncLocalStorage is Node.js-only. In browser builds, this stays null.
// The original code used top-level await to dynamically import node:module,
// which is incompatible with Vite's production build target (es2020).
// Since this code runs in the browser, AsyncLocalStorage is never available.
...
// In browser builds, AsyncLocalStorageClass is always null, so executionContext is null.
const executionContext: AsyncLocalStorageType<DefenseContext> | null =
	!IS_BROWSER && AsyncLocalStorageClass ? new (AsyncLocalStorageClass as new () => AsyncLocalStorageType<DefenseContext>)() : null;
