...
import { applyCaseTransform, getLocalVarDepth, isInteger } from "./builtins/index";
import { ArithmeticError, ExitError } from "./errors";
import { expandWord, expandWordWithGlob, getArrayElements } from "./expansion";
import { parseKeyedElementFromWord, wordToLiteralString } from "./helpers/array";
import { getNamerefTarget, isNameref, resolveNameref, resolveNamerefForAssignment } from "./helpers/nameref";
...
async function processScalarAssignment(
	...
	if (isInteger(ctx, targetName)) {
		...
		const { isArray } = await import("./expansion");
